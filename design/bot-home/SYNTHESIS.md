...
Base: candidate 2 (Chief is a compile-time constant, not a sqlite row).
Cross-judge recommended candidate 1. Parent picked 2.

## Rubric

1. Chief cannot be omitted from a loaded roster.
...
The judge scored both Strong on Chief-unomittable, then flagged C1 for needing delete/rename policy on a Chief row. That leak is the reason C2 wins. A wiped `teammates` table still loads Chief. Talk can add a thread later; it does not invent Chief storage.
...
From C2: named `chief` field on `Roster`.
From the prior live branch: first paint is `chiefOnlyRoster()`, not a loading canvas.

## Rejected

- Chief sqlite row (C1). Identity would be data.
...
- `role: 'lead' | 'teammate'` on the object. `Chief` vs `Teammate` already discriminates.
...
Empty loaded roster is unrepresentable. `select('chief')` does not write when already current. Box is not in the module map. First paint types as a roster that includes Chief.
