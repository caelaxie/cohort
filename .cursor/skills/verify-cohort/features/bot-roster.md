...
Home is the named-bot roster. Hatch is the lead bot on launch. The Crew sidebar lists Hatch first. The main pane shows Hatch's name, an empty thread, and a Message composer.

## Sub-features

- `launch-hatch` shows Hatch as current on a fresh home.
- `crew-heading` labels the sidebar `Crew`.
- `empty-thread` shows Hatch's name in the `h1`, a Message composer, and no sandbox badge or files rail.
...
- **Empty thread.** Run `... eval --port $PORT --js "document.querySelector('h1')?.textContent"`. It prints `"Hatch"`. Run `... eval --port $PORT --js "[...document.querySelectorAll('textarea')].length"`. It prints `1`. The snapshot includes `Message` and `Send`. It does not show `New`, `Add files`, or a `Files` rail.
