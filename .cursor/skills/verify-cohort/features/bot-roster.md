...
- **State.** Run `... state --port $PORT --path "$RUN_ROOT/evidence/bot-roster/state.json"`. `roster.hatch.id` is `hatch`, `roster.hatch.name` is `Hatch`, `roster.current` is `hatch`, `roster.others` is `[]`, and `thread.messages` is `[]`. Then outside the UI: `sqlite3 "$RUN_ROOT/home/state.sqlite" "SELECT key, value FROM meta WHERE key='current_id'; SELECT id, name FROM teammates;"` shows `current_id|hatch` and no teammate rows.
...
- **Reopen.** Tear down the instance per `../SKILL.md` cleanup but keep `$RUN_ROOT/home`. Relaunch with the same `COHORT_HOME=$RUN_ROOT/home`, then `doctor`. `... state --port $PORT` still has `roster.current` of `hatch` and `roster.hatch.name` of `Hatch`. The snapshot still shows Crew and `h1 "Hatch"`.
