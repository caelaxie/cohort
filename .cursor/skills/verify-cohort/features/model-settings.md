...
- **Paste connect.** Use the Base URL, Model, and API key from `../SKILL.md` **Model provider**. Run `... fill --port $PORT --label "Base URL" --value "http://127.0.0.1:11434/v1"`, then `... fill --port $PORT --label "Model" --value "qwen3.8:27b-mlx"`, then `... fill --port $PORT --label "API key" --value "ollama"`, then `... click --port $PORT --text "Connect"`. Wait until a `p` reads `qwen3.8:27b-mlx`. Then outside the UI: `cat "$RUN_ROOT/home/prime/agent/auth.json"` contains `"openai-completions"`, `"http://127.0.0.1:11434/v1"`, and `"qwen3.8:27b-mlx"`. The owner's `~/.prime/agent/auth.json` is not this file.
