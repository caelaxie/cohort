...
- **State.** `... state --port $PORT` still has `current` of `hatch`. `sqlite3 "$RUN_ROOT/home/talk.sqlite" "SELECT owner_body, bot_body FROM turns ORDER BY created_at, owner_id;"` shows `hello|hi from Hatch`.
