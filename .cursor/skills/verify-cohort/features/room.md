...
- For a completed send, connect local Ollama per `../SKILL.md` **Model provider**.
...
- **Owner picks Chief.** With To on Chief, `... fill --port $PORT --label "Message" --value "Reply with only the word pong."`, then `... click --port $PORT --text "Send"`, then `... wait --port $PORT --timeout 120000 --js "[...document.querySelectorAll('[data-speaker=\"chief\"] p')].some(p => /pong/i.test(p.textContent||''))"`. Speaker labels are You then Chief.
- **Owner picks teammate.** Set To to Scout, send `Reply with only the word pong.`. Wait `--timeout 120000` until `data-speaker` includes `scout`. Speaker label for that reply is Scout.
- **Persist.** `sqlite3 "$RUN_ROOT/home/talk.sqlite" "SELECT speaker_kind, speaker_bot_id, body FROM room_lines ORDER BY n;"` includes `owner||Reply with only the word pong.`, a `bot|chief` line containing `pong`, and a Scout reply. `sqlite3 "$RUN_ROOT/home/talk.sqlite" "SELECT bot_id FROM turns;"` does not gain room rows.
